...
- **Black filled — observed** phase lags from the previous London-wavelet plot, the same downward-trending cloud from $\sim 0^\circ$ near London out to $\sim -45^\circ$ at distance 30.
- **Light-blue dots — simulated** phase lags for the named simulation, with the title's `sim=` value reporting that simulation's squared-difference score against the observed cloud.
