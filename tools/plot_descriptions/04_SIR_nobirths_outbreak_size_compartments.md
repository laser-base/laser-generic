...
- **Blue solid — Susceptible $S_t$.** Starts at $10^5$, holds flat until ~day 40, drops steeply through the epidemic, and settles at $S_\infty \approx 6{,}000$.
- **Red solid — Infected $I_t$.** Rises from the seed, peaks near $\approx 30{,}000$ around day 80, and decays back to zero by ~day 200.
- **Green solid — Recovered $R_t$.** Mirrors $S$ as it climbs from 0 to a plateau just below $10^5$.
- **Orange dashed — `newly_recovered` $\Delta R_t$.** A small bump tracking the recovery flux; peaks well below the $I_t$ peak.
- **Gray dashed — analytic $R(\infty)$** from the Kermack-McKendrick / Lambert-W solution, sitting almost exactly on top of the green plateau.
