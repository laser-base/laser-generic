...
- **Blue dots — one point per simulation** across a 100-run sweep over $\sigma$ logarithmically spaced.
- **Orange line — fitted curve** $C = \beta + \sigma / (\xi + \sigma)$ with $\xi \approx 0.0133$ and $\beta \approx 0$, as labelled in the legend.
