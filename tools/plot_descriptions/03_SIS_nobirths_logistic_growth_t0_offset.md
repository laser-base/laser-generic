...
- **"Model output"** (blue, solid, thick): the stochastic SIS trajectory from this single realization.
- **"Logistic growth with known inputs, t0=0"** (orange, solid): the analytic SIS logistic $\frac{Nx}{1+(Nx/I_0-1)e^{-\beta x t}}$ with $x = 1-\gamma/\beta$, evaluated at the known $\beta = 0.1$, $\gamma = 1/32$, and no time offset.
- **"Logistic growth with known inputs, best-fit t0 = 36"** (red, dotted): the same analytic curve with a best-fit time offset $t_0 \approx 36$ days.
