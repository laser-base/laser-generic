...
- **Filled markers** — the 200 sims (after filtering the floor/ceiling cases), coloured by $R_0$ on a viridis scale (dark ~2, yellow ~15).
- **Four straight diagonal lines** — the fitted relationship evaluated at $R_0 \in \{1.5, 3, 8, 16\}$. All four have the same slope (~1.83, the recovered $\alpha$ exponent) and are simply offset vertically by the $(R_0/(R_0-1))^{1.67}$ factor: the blue $R_0=1.5$ line sits highest, then orange ($R_0=3$), then green ($R_0=8$) and red ($R_0=16$) nearly on top of each other near the bottom.
