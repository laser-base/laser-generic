...
- **Sky-blue histogram** of non-zero `initial_ri` values, 30 bins spanning roughly 210–340 days. The shape is a clean bell with peak counts of ~2,000 near the center bins.
- **Red dashed vertical line** at the empirical mean, labeled "Mean 273.5".
- **Dark-blue solid curve** — the analytic `Normal(274, 15)` PDF, rescaled by `nonzero_initial_ri.size * bin_width` so its peak matches the histogram. It hugs the bar tops across the full range.
