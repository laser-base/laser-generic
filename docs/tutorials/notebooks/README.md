...
Notebook execution (~30 min) is expensive, so Execute Notebooks only runs when the notebook, library, or dependency source actually changes. On pushes that don't affect notebook execution — an `.md` doc edit, a mkdocs config tweak, an unrelated code refactor — MkDocs Deploy skips re-execution entirely and reuses the most recently produced `executed_nbs` artifact, so the site rebuild is fast (~2 min). Only pushes that touch notebook or library source pay the full execution cost, and even then the source-hash cache in Execute Notebooks makes a second push with the same source finish in seconds.
