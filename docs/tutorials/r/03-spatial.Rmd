---
title: "Spatial models with laser-generic"
...
populations live in multiple geographic nodes and couple them with
a migration network. `laser.generic` builds a **gravity** network
automatically from each node's population and centroid coordinates;
this tutorial walks through three concrete uses of that machinery and
shows how to override the network when you want a custom topology.
...
A factory that builds an SIR scenario over a grid of nodes and seeds
the disease in a specified node. Used by every example below.
...
> network as `row_normalizer(gravity(...), max_rowsum = (1/16) + (1/32))`.
> The `row_normalizer` step caps each row sum at ≈0.094 — a hard limit
...
> the *raw* gravity values blow past the cap for any reasonable `k`,
> so all of `k = 50`, `500`, `5000` produce **identical** normalized
...
```{r two-patch-sweep, fig.height = 5}
seed_everything()
...
- At `w = 0.01` coupling is strong enough to seed node 1 within the
  first couple of weeks; the two outbreaks run nearly in parallel with
  a clear but modest lag.
...
```{r custom-networks, fig.height = 6}
seed_everything()
...
- **chain** — disease ripples 0 → 1 → 2 → 3 with visible lag at each hop.
...
```{r network-matrices, fig.width = 8.5, fig.height = 2.5}
mats <- list(
...
- **Empirical mobility matrices.** Substitute a row-normalized call-data
  matrix (or any other empirically calibrated network) for `model$network`
  with the same `set_network()` helper used above.
- **Gravity parameter calibration.** Real-world studies usually fit
  ...
  individual gravity parameters lose visibility. If that bites,
  override `model$network` directly (as we do in the two-patch and
