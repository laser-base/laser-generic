...
- [The reintroduction component](#the-reintroduction-component)
...
- [Scaling up: measles-style biennial
  dynamics](#scaling-up-measles-style-biennial-dynamics)
...
1.  **Endemic-equilibrium initial conditions.** Instead of starting with
    ...
    of an endemic disease: `S = N / R0` and `R = N · (1 - 1 / R0)`, with
    `E = I = 0`. The disease enters the population only via
    reintroduction.
2.  **Periodic reintroduction throughout.** A small custom component
    ...
    `S` and `R` populations evolve and how that drives the subsequent
    disease dynamics.
...
2.  **Higher R0** (12 vs 5) — within the canonical measles range. Each
    wave consumes nearly all available susceptibles, and the
...
3.  **Faster, more aggressive reintroduction** — 5 cases every six weeks
    instead of every two months, and started a few months in to let the
    equilibrium settle before the dynamics start swinging.
...
We run four configurations under the same measles-scale setup and stack
them on one axis so the differences between vital-dynamics choices read
off at a glance.
