...
tick is computed against today's population, not after the newborns
have joined.
...
```{r mortality-by-cdr-with-births}
seed_everything()
...
`track = TRUE` so newborns have a recorded `dob`.

```{r mortality-by-estimator}
seed_everything()
...
the trajectory are driven by importation versus by the original
seeded prevalence.
