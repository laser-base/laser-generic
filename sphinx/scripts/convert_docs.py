#!/usr/bin/env python3
"""
convert_docs.py — Convert Markdown docs to reStructuredText (RST)
...
def generate_auto_toc(rst_dir: pathlib.Path, AUTO_TOC_FILENAME: str):
    ...
    all_rst = sorted(
        [p for p in rst_dir.rglob("*.rst") if p.name != AUTO_TOC_FILENAME]
    )
    ...
    log(f"Wrote explicit TOC with {len(all_rst)} entries → {toc_path}")
...
def main():
    start = time.time()
    log("Starting Markdown → RST conversion.")
    log(f"Source: {md_dir}")
